...
def file2matrix(filename):
    ...
    for line in arrayOLines:
        ...
        if listFromLine[-1] == 'fail':
            classLabelVector.append(1)
        elif listFromLine[-1] == 'Marginal':
            classLabelVector.append(2)
        elif listFromLine[-1] == 'excellent':
...
def showdatas(datingDataMat, datingLabels):
    ...
    plt.setp(axs2_ylabel_text, size=7, weight='bold', color='black')
     ...
     # 画出散点图,以datingDataMat矩阵的第二(玩游戏)、第三列(冰激凌)数据画散点数据,散点大小为15,透明度为0.5
    axs[1][1].scatter(x=datingDataMat[:, 1], y=datingDataMat[:, 2], color=LabelsColors, s=15, alpha=.99)
    # 设置标题,x轴label,y轴label
    axs3_title_text = axs[1][1].set_title(u'每次作业平均完成比例与每天上课平均坐前排平均次数', FontProperties=font)
    axs3_xlabel_text = axs[1][1].set_xlabel(u'每次作业平均完成情况比例', FontProperties=font)
    axs3_ylabel_text = axs[1][1].set_ylabel(u'每天上课平均坐前排平均次数比值', FontProperties=font)
    plt.setp(axs2_title_text, size=9, weight='bold', color='red')
    plt.setp(axs2_xlabel_text, size=7, weight='bold', color='black')
    plt.setp(axs2_ylabel_text, size=7, weight='bold', color='black')
    # 设置图例
    fail   = mlines.Line2D([], [], color='black', marker='.',
                              markersize=6, label='fail')
    Marginal   = mlines.Line2D([], [], color='orange', marker='.',
                               markersize=6, label='Marginal')
    excellent  = mlines.Line2D([], [], color='red', marker='.',
                               markersize=6, label='excellent')
    # 添加图例
    axs[0][0].legend(handles=[fail,Marginal,excellent])
    axs[0][1].legend(handles=[fail,Marginal,excellent])
    axs[1][0].legend(handles=[fail,Marginal,excellent])
    axs[1][1].legend(handles=[fail,Marginal,excellent])
    # 显示图片
    plt.show()
...
def classifyPerson():
    ...
    zuoCream = float(input("每次作业平均完成情况比例:"))
    # 打开的文件名
    filename = "test.txt"
    # 打开并处理数据
    datingDataMat, datingLabels = file2matrix(filename)
    ...
    # 生成NumPy数组,测试集
    inArr = np.array([precentTats, ffMiles, iceCream,zuoCream])
    # 测试集归一化
    ...
    # 打印结果
    print("这名同学整体成绩可能%s" % (resultList[classifierResult-1]))
